...
class GINN(object):
    ...
    def __init__(
        ...
        maskT = torch.HalfTensor(self.mask).to(self.device)
        nxg = dataset2nxg(
            self.features,
            self.mask,
            self.percentile,
        ...
        g.ndata["norm"] = torch.HalfTensor(norm).to(self.device)
        self.graph = g
        ...
        if skip:
            ...
            norm[torch.isinf(norm)] = 0
            norm = norm.unsqueeze(1)
            f.ndata["norm"] = torch.HalfTensor(norm).to(self.device)
            self.f = f
            if glob_attr:
                m = torch.HalfTensor(np.mean(self.features, axis=0)).to(self.device)
    ...
    def add_data(self, new_features, new_mask, new_num_mask, new_cat_mask):
        ...
        norm[torch.isinf(norm)] = 0
        norm = norm.unsqueeze(1)
        self.graph.ndata["norm"] = torch.HalfTensor(norm).to(self.device)
        ...
        if self.f:
            ...
            norm[torch.isinf(norm)] = 0
            norm = norm.unsqueeze(1)
            self.f.ndata["norm"] = torch.HalfTensor(norm).to(self.device)
    ...
    def fit(
        ...
        featT = torch.HalfTensor(self.features).to(self.device)
        maskT = torch.HalfTensor(self.mask).to(self.device)
            ...
            if adversarial:
                if glob_attr:
                    ag_loss = a_loss + global_criterion(
                        torch.HalfTensor(np.mean(self.features, axis=0)).to(
                            self.device
                        ),
                        torch.HalfTensor(
                ...
                for _ in range(crit_iter):
                    ...
                    x_fake = torch.HalfTensor(x_fake).to(self.device)
